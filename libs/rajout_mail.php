...
define('MAIL_USERNAME', 'contact@gestion-cnb.org');
define('MAIL_PASSWORD', 'Cnb+258789+*');
...
function envoyerUnMail($destinataires, $sujet, $corps, $fichier = null)
{
    if (!empty($destinataires) && !empty($sujet) && !empty($corps)) {
        ...
        // $mail->SMTPDebug = 2;
        $mail->isSMTP();
        $mail->Host = 'mail.gestion-cnb.org';
        $mail->Port = 465;
        $mail->SMTPAuth = 1;
        ...
        $mail->From       =  'contact@gestion-cnb.org';                //L'email à afficher pour l'envoi
