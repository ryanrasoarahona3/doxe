...
class Document
{
    ...
    public function creation_DON() 
    {
		...
		// Vérifier si la commande contient bien un don
		/*
		$don=0;
		foreach ($order->commerce_line_items['und'] as $line) {
  			$line_item = commerce_line_item_load($line['line_item_id']);
  			$produit = commerce_product_load($line_item->commerce_product['und'][0]['product_id']);
  			if ($line_item->type == 'donation') {	
				$don = $don + $line_item->commerce_unit_price['und'][0]['amount'];
			} 
		}	
		
		// Vérification du total
		if ($don == 0) {
			// echo 'Demande incorrecte pas de donation';
        	return false;
		}
		*/
