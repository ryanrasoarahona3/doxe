...
if ( (isset($_GET['retour'])) && (!empty($_SESSION['recherche'][$recherche])) ) {
	...
	$produit = getSelect('commerce_produits' ,$choixRecherche['produit']);
	$etats = getSelect('commerce_payement_etat',$choixRecherche['etat']);
} else {
	$produit = getSelect('commerce_produits' );
	$etats = getSelect('commerce_payement_etat' );
}
